...
std::optional<SortRegion> SortSession::selectRegion(ContainerScreenController& controller, Config const& config) {
    auto const& manager = controller.mContainerManagerController;
    if (!manager) return std::nullopt;

    if (config.sortContainers && isOrdinaryStorage(manager->getContainerType())) {
